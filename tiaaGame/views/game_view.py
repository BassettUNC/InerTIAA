...
from tiaaGame.message_box import MessageBox
from tiaaGame.player import Player as player
from tiaaGame.sprites.player_sprite import PlayerSprite
...
class DebugMenu(arcade.gui.UIBorder, arcade.gui.UIWindowLikeMixin):
                    ...
                    arcade.gui.UIAnchorWidget(
                        child=arcade.gui.UIBoxLayout(
                            ...
                            children=[
                                ...
                                arcade.gui.UIPadding(
                                    child=self.money_button, padding=(5, 5, 5, 5)
                                ),
                                arcade.gui.UIPadding(
                                    child=self.work_button, padding=(5, 5, 5, 5)
                                ),
                            ],
                            vertical=False,
                        ),
    ...
    def setup_noclip(self, callback: Callable):
        ...
        # Setup our other debug buttons
        self.money_button = arcade.gui.UIFlatButton(
            text="money", style=self.off_style
        )
        self.work_button = arcade.gui.UIFlatButton(
            text="work", style=self.off_style
        )
        self.money_button.on_click = self.on_click_money
        self.work_button.on_click = self.on_click_work

    def on_click_money(self, event):
        player.balance += 1000000
    def on_click_work(self, event):
        player.workProgress()
...
class GameView(arcade.View):
    ...
    def draw_inventory(self):
        vertical_hotbar_location = 25
        hotbar_height = 50
        sprite_height = 16

        field_width = self.window.width / (5)

        x = self.window.width / 2
        y = vertical_hotbar_location

        arcade.draw_rectangle_filled(
            x, y, self.window.width, hotbar_height, constants.LIGHTER_BLUE
        )
        i = 0
        while i < 5:
            y = vertical_hotbar_location - 9
            x = i * field_width + 5
            bold = False
            ...
            if i == 0:
                ...
                item_name = "Age: " + str(player.age)
                
            elif i == 2:
                item_name = str(player.date)
            elif i == 3:
                item_name = "$" + str(int(player.balance))
            elif i == 4:
                item_name = "Save"
                bold = True
            ...
            arcade.draw_lrtb_rectangle_outline(
                    x -6 , x + field_width -6, y + 32, y - 25, constants.DARK_BLUE, 2
            )
            arcade.draw_text(text, x, y, constants.DARK_BLUE, constants.SMALL_FONT_SIZE, font_name=constants.DEFAULT_FONT, bold = bold)
            i += 1
    ...
    def popupError(self, description: str):
        print(description)
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key in constants.KEY_UP:
            ...
            self.window.show_view(self.window.views["marketplace"])
        elif key == arcade.key.KEY_2:
            self.window.show_view(self.window.views["car_dealership"])
        elif key == arcade.key.KEY_3:
            self.window.show_view(self.window.views["real_estate"])
